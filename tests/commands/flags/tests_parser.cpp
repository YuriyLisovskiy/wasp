/**
 * commands/flags/tests_parser.cpp
 *
...
#include "../../../src/commands/flags/parser.h"
...
TEST(TestCase_args_parser, parseValid)
{
	...
	cmd::flags::ArgumentsParser parser((int)argv.size() - 1, argv.data(), 1, false);
...
TEST(TestCase_args_parser, parseInvalidValid)
{
	...
	cmd::flags::ArgumentsParser parser((int)argv.size() - 1, argv.data(), 0, false);
	std::map<std::string, std::string> expected = {
