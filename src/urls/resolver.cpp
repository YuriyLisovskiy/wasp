...
std::function<std::unique_ptr<http::abc::HttpResponse>(http::Request*, conf::Settings*)> resolve(
	const std::string& path, const std::vector<std::shared_ptr<IPattern>>& urlpatterns
)
{
	std::function<std::unique_ptr<http::abc::HttpResponse>(http::Request*, conf::Settings*)> fn = nullptr;
