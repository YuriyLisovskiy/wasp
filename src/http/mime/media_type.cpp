...
void guess_content_type(const std::string& _path, std::string& type, std::string& encoding)
{
	...
	else
	{
		...
		{
			path::split_text(base + _get_from_string_map(SUFFIX_MAP, ext, ""), base, ext);
		}

		encoding = _get_from_string_map(ENCODINGS_MAP, ext, "");
		//	path::split_text(base, base, ext);

		std::string lower_ext = str::to_lower(ext);
		if (TYPES_MAP.contains(ext))
		{
			type = _get_from_string_map(TYPES_MAP, ext, "");
		}
		else
		{
			type = _get_from_string_map(TYPES_MAP, lower_ext, "");
		}
	}
}
